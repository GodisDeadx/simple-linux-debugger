...
void debugger::print_backtrace() {
    auto output_frame = [frame_number = 0](auto&& func) mutable {
        std::cout << "frame #" << frame_number++ << ": 0x" << dwarf::at_low_pc(func) << ' ' << dwarf::at_name(func) << '\n';
    };

    auto current_func = get_function_from_pc(offset_load_address(get_pc()));
    output_frame(current_func);

    auto frame_pointer = debug::get_register_value(m_pid, debug::reg::rbp);
...
uint64_t debugger::get_pc() {
    return debug::get_register_value(m_pid, debug::reg::rip);
}
...
void debugger::set_pc(uint64_t pc) {
    debug::set_register_value(m_pid, debug::reg::rip, pc);
}
...
void debugger::dump_registers() {
    for (const auto& rd : debug::g_register_descriptors) {
        std::cout << rd.name << " 0x"
                  << std::setfill('0') << std::setw(16) << std::hex << debug::get_register_value(m_pid, rd.r) << std::endl;
    }
}
...
void debugger::handle_command(const std::string& line) {
    ...
    if (is_prefix(command, "cont")) {
        ...
        std::cout << debug::get_register_value(m_pid, debug::get_register_from_name(args[2])) << std::endl;
    }
    else if (is_prefix(args[1], "write")) {
        std::string val {args[3], 2}; //assume 0xVAL
        debug::set_register_value(m_pid, debug::get_register_from_name(args[2]), std::stol(val, 0, 16));
    }
